...
export default function BroadcastMedia() {
    ...
    const GetAPICards = async () => {
      ...
      setGo("ok")
      setLoading(true)
      await axios.get("https://fakerapi.it/api/v1/products?_price_max=100&_quantity=4&_price_min=40&_type=architecture")
              .then((result)=>{
                  setCards([
                    ...result.data.data
                  ])
  ...
  return ( 
    <KeyboardAvoidingView style={{display:"flex", justifyContent:"start", alignItems:"start", height:"100%"}}>
      <HeaderCat txt='Business & Finance' url='https://starbooks.ca/wk06website/wp-content/uploads/2022/12/business-Small.jpeg'></HeaderCat>
