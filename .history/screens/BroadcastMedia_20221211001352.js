...
export default function BroadcastMedia() {
    ...
    const GetAPICards = async () => {
      ...
      await axios.get("https://fakerapi.it/api/v1/products?_price_max=130&_quantity=6&_price_min=40")
              .then((result)=>{
                  setGo(true)
                  setCards([
                    ...result.data.data
    ...
    <ScrollView style={{marginTop:50}}>
        {go === true && cards.map((o,i)=>( 
          <View key={i} style={{marginBottom:30}}>
            <PostCard url={o.image} title={o.name} description={o.price} ratings={"Ratings: " + o.categories[0] + "/10 Stars"}/>
          </View>
        ))}
    </ScrollView>
