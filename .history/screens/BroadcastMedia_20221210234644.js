...
export default function BroadcastMedia() {
    ...
    const GetAPICards = async (i) => {
      ...
      await axios.get("https://makeup-api.herokuapp.com/api/v1/products.json?brand=colourpop")
              .then((result)=>{
                  // setGo(true)
                  // setCards([
                  //   ...result
  ...
  return ( 
    ...
    <Button onPress={() => GetAPICards(i)} title="Get Fake API"/>
    <ScrollView style={{marginTop:50}}>
        {go === true && cards.map((o,i)=>( 
          setCocoa
          <View key={i} style={{marginBottom:30}}>
            <PostCard url={o.image_link} title={o.name} description={o.price}/>
          </View>
        ))}
    </ScrollView>
