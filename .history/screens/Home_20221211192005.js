...
export default function Home() {
  ...
  return (
     <View style={{borderWidth:0, width:"100%", padding:"20px", height:"100%", backgroundColor:"#F8F6F2"}}>
          <View style={{width:"100%", justifyContent:"start", alignItems:"flex-start", paddingTop:50, paddingRight:0, paddingLeft:20, minHeight:100, borderBottomWidth: 1, borderBottomColor: 'grey', marginBottom:20}}>
            <Text style={{fontSize:20, fontWeight:"700", minHeight:100, textAlignVertical:"bottom"}}>Search</Text>
          </View>
          <View style={{flexWrap:"wrap", flexDirection:"row", borderWidth:"0px", display:"flex", justifyContent:"center"}} >
          {listOfCategories.map((category) => (
              <CategoryCard key={category.id} onPress={()=>{SendRoute(category.id)}} name={category.name} image={category.image}/>
            ))
          }
          </View>
      </View>
