...
export default function BusinessFinance() {
    ...
    const GetAPICards = async () => {
      ...
      setGo("ok")
      setLoading(true)
      await axios.get("https://fakerapi.it/api/v1/products?_price_max=200&_quantity=15&_price_min=40&_type=people")
              .then((result)=>{
                  setCards([
                    ...result.data.data
                  ])
  ...
  return ( 
    <KeyboardAvoidingView style={{display:"flex", justifyContent:"start", alignItems:"start", height:"100%"}}>
      <HeaderCat txt='Digital Arts & Design' url='https://starbooks.ca/wk06website/wp-content/uploads/2022/12/digitalarts-Small.jpeg'></HeaderCat>
